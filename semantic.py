...
so the client looks up bbox/page directly from /api/jobs/<id>/words/<side>.
...
def build_semantic_map(segments, mapped, old_words, new_words):
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        if owid is None or nwid is None:
            continue  # need both sides for a both-side anchor
        equal_words.append({
            "old_word_id": owid,
            "new_word_id": nwid,
...
# =========================================================
# resolution: dragged word_ids -> equal chunk
# =========================================================

def resolve_word_ids(semantic_map, side, word_ids):
    """Keep only equal words of the dragged selection; drop changed ones."""
...
def attach_index_old_side(index_items, semantic_map, old_words, new_words):
    """Give each (new-PDF) index item its matching old-PDF page/bbox via equal words."""
...
# =========================================================
# reviews storage (runs/<job_id>/reviews.json)
# =========================================================
...
def save_reviews(run_dir, reviews):
    _reviews_path(run_dir).write_text(
...
def add_review(run_dir, semantic_map, data):
    """
    data = {side, page, rect, word_ids:[raw dragged ids on `side`], comment}
    Server resolves to the equal chunk so changed words are ignored.
    Returns the review, or {"error": ...} when no unchanged word was selected.
    """
    side = data.get("side")
    ...
    review = {
        ...
        "change_ids": [],            # reviews target unchanged content by design
...
# =========================================================
# AI input bundle (structure only - no AI call)
# =========================================================
