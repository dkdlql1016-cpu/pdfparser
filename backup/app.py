...
def detect_notes_start_page(new_md_path: Path, visual_lines):
    # Prefer PDF visual lines because they are real page locations.
    for ln in visual_lines:
...
def search_sequence(words, needle_tokens):
    ...
    for i in range(0, len(norms) - n + 1):
        if norms[i:i+n] == needle_tokens:
            ...
            return {
                "page": matched[0]["page"],
                "bbox": line_bbox(matched),
                "anchor_text": line_text(matched),
            }
    return None
...
def find_note_anchor_in_words(note_no, title, new_words, preferred_page=None, notes_start_page=None):
    ...
    # 1) note number + title prefix. This prevents title-only hits in audit report/table text.
    ...
    # 2) title-only only near the MD page hint. Never global-first.
    if preferred_page:
        near = page_words([preferred_page - 1, preferred_page, preferred_page + 1])
        for title_len in range(min(10, len(title_tokens)), 0, -1):
    ...
    # 3) fallback to first word on preferred page / notes start page.
    fallback_page = preferred_page or notes_start_page or 1
...
def build_new_pdf_index(new_md_path: Path, new_words, page_count):
    ...
    # Keep navigation in document order, and notes naturally by anchor page/order.
    ...
    for items in groups.values():
        ...
        for line in lines:
            ...
            for h in line["items"]:
                x0, y0, x1, y1 = h["bbox"]
                if chunk is None:
                    chunk = {"ids": [h["id"]], "texts": [h.get("text", "")], "change_ids": [h["change_id"]], "type": h["type"], "page": h["page"], "x0": x0, "y0": y0, "x1": x1, "y1": y1}
                    continue
                gap = x0 - chunk["x1"]
                if gap <= max_gap:
...
def alignment_report(segments, old_words, new_words):
    ...
    old_map, new_map = align_stream_to_pdf_words(old_stream, old_words), align_stream_to_pdf_words(new_stream, new_words)
    return {
...
def process_job(job_id, old_pdf, new_pdf):
    ...
    try:
        ...
        viewer_data = {
            "job_id": job_id,
            "app_version": "index-md-first-v6",
            "index_strategy": "md_page_marker_pdf_note_anchor_no_disk_fallback",
...
def get_job(job_id):
    # Do NOT read runs/<job_id>/viewer_data.json here.
    # Old runs are archive only; returning them here makes the UI show stale results.
    job = JOBS.get(job_id)
    if not job:
