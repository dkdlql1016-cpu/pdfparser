...
def remove_page_marker_segments(segments):
    ...
    for i, s in enumerate(segments):
        ...
        if st in ("delete", "add") and is_page_num_token(t):
            ...
            pair_num = (typ(i - 1) in ("delete", "add") and is_page_num_token(txt(i - 1))) or (
...
def diff_with_difflib(old_tokens, new_tokens):
    sm = SequenceMatcher(None, [x[0] for x in old_tokens], [x[0] for x in new_tokens], autojunk=False)
    raw = []
...
def diff_with_git(old_tokens, new_tokens, algorithm):
    ...
    try:
        ...
        if proc.stdout.strip():
            ...
            raw = [{"type": "equal", "old_idx": i + 1, "new_idx": i + 1} for i in range(min(len(old_tokens), len(new_tokens)))]
        return remap(raw, old_tokens, new_tokens)
    finally:
...
def main():
    ...
    ap.add_argument("--diff-algorithm", choices=["histogram", "myers", "patience", "minimal"], default="histogram")
